...
export class MockAiProvider implements AiProvider {
  async summarizeTicket(ticket: TicketContext): Promise<string> {
    ...
    const latest = last
      ? stripHtml(last.body).slice(0, 160)
      : 'No messages yet.';
  ...
  async suggestPriority(input: {
    ...
    if (URGENT_WORDS.some((w) => lower.includes(w))) {
      return {
        ...
        reasoning:
          'Contains language suggesting a critical/production-impacting issue.',
      };
    }
    if (HIGH_WORDS.some((w) => lower.includes(w))) {
      return {
        priority: TicketPriority.HIGH,
        reasoning: 'Describes something actively broken or blocking.',
      };
    }
    return {
      priority: TicketPriority.MEDIUM,
      reasoning:
        'No urgency signals detected; defaulting to standard priority.',
    };
  }
  ...
  async suggestTags(input: {
    ...
    const words = Array.from(wordSet(`${input.subject} ${input.body}`)).slice(
      0,
      6,
    );
    ...
    for (const word of words) {
      const reused = existingLower.find(
        ...
        .map((c) => ({
          ...
          confidence:
            Math.round(
              jaccard(
                targetWords,
                wordSet(`${c.subject} ${c.firstCommentBody}`),
              ) * 100,
            ) / 100,
    ...
    return (
      candidates
        .map((c) => ({
          ...
          confidence:
            Math.round(
              jaccard(targetWords, wordSet(`${c.title} ${c.excerpt}`)) * 100,
            ) / 100,
